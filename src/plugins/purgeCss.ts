...
interface VitePurgeCSSOptions extends Omit<PurgeCSSOptions, 'content' | 'css'> {
  enabled?: boolean | ((mode: string) => boolean) // Whether to enable in development mode

  verbose?: boolean // Whether to log statistics
}

export function purgeCSS(options: VitePurgeCSSOptions = {}): Plugin {
  const { enabled = (mode) => mode === 'production', verbose = false, ...purgeCSSOptions } = options
    ...
    async generateBundle(_, bundle) {
      ...
      try {
        ...
        for (const fileName in bundle) {
          ...
          if (fileName.endsWith('.map')) continue // Skip source maps

          if (chunk.type === 'asset' && fileName.endsWith('.css')) {
            cssFiles.push({ name: fileName, source: String(chunk.source) })
          } else if (
            ...
            contentFiles.push({ name: fileName, source })
          }
        }
        ...
        const purgeOptions: PurgeCSSOptions = {
          ...
          css: cssFiles.map((file) => ({ raw: file.source })),
        }
